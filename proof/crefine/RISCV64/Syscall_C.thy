...
lemma handleInvocation_def2:
  "handleInvocation isCall isBlocking canDonate firstPhase cptpr =
      ...
      case v of
        ...
        | Inr (slot, cap, extracaps, buffer) \<Rightarrow>
               do args \<leftarrow> getMRs thread buffer info;
                  v' \<leftarrow> do v \<leftarrow> RetypeDecls_H.decodeInvocation (msgLabel info) args ptr slot cap extracaps firstPhase buffer;
