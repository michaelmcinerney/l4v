...
proof (intro impI allI)
  ...
  (* s/obj/obj'/ *)
  (* FIXME RT: requires the padding for the notification to be corrected *)
  have szo: "size_of TYPE(notification_C) = 2 ^ objBitsKO ko"
    sorry (* (simp only: size_of_def objBits_simps' ko_def) *)
...
find_theorems (89) makeObject
term createObject
thm createObject_def
typ tcb
  ...
  apply (cinit lift: t_' regionBase_' userSize_' deviceMemory_')
   ...
   apply (subgoal_tac "\<exists>apiType. newType = APIObjectType apiType")
    ...
    apply (rule ccorres_guard_imp)
      ...
      apply simp
     apply auto[1]
(* FIXME RT: resolve preconditions from TCB case *)
sorry (*
    apply (clarsimp simp: createObject_c_preconds_def)
