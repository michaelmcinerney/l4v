...
(* FIXME RT: unused? This might require an additional assumption that thread is not
   the idle thread, and some further reasoning involving valid_idle' *)
lemma rf_sr_tcb_update_not_in_queue:
  "\<lbrakk> (s, s') \<in> rf_sr; ko_at' tcb thread s;
    ...
    \<not> live' (KOTCB tcb); invs' s; sym_refs (state_refs_of' s);
    (\<forall>x\<in>ran tcb_cte_cases. (\<lambda>(getF, setF). getF tcb' = getF tcb) x);
