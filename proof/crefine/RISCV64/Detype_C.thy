...
proof -
  ...
  show "pspace_no_overlap' (ptr + of_nat idx) bits s"
    ...
    by (force simp:ko_wp_at'_def obj_range'_def p_assoc_help pspace_bounded'_def)
  qed
...
lemma cmap_relation_disjoint:
  ...
  and    sof: "\<And>ko v. proj ko = Some v \<and> koType TYPE('a) = koTypeOf ko \<Longrightarrow>
                      size_td (typ_info_t TYPE('b)) \<le> 2 ^ objBitsKO ko"
...
proof -
  ...
  proof (rule upto_intvl_eq)
    ...
    with ks show "is_aligned (ptr_val x) (objBitsKO ko)"  by (fastforce intro: pspace_alignedD')
  qed
...
proof -
  ...
  next
    ...
    hence koat: "ko_at' tcb (ptr_val x - n) s"
      apply -
      apply (erule obj_atI', simp_all add: objBits_simps' word_bits_def)
      done
...
proof -
  ...
  proof (rule upto_intvl_eq)
    from invs have "pspace_aligned' s" ..
    with ks show "is_aligned (ptr_val x) (objBitsKO KOUserData)" by (fastforce intro: pspace_alignedD')
  qed
...
proof -
  ...
  proof (rule upto_intvl_eq)
    from invs have "pspace_aligned' s" ..
    with ks show "is_aligned (ptr_val x) (objBitsKO KOUserDataDevice)"  by (fastforce intro: pspace_alignedD')
  qed
...
thm schedContextStructSize_def
definition sizeof_sched_context_t :: nat where "sizeof_sched_context_t = 96"

definition schedContextStructSize :: nat where "schedContextStructSize = 96"
...
proof -
  ...
  moreover have "ceil_log (sizeof_sched_context_t) = 7"
    apply (simp add: sc_const_eq(2) ceil_log_def sizeof_sched_context_t_def)
    by (fastforce intro!: discrete_log_eqI)
  ultimately show ?thesis
    by (clarsimp simp: scBitsFromRefillLength'_def max_num_refills_def sizeof_sched_context_t_def
refill_size_bytes_def schedContextStructSize_def)
qed
...
sorry
  ...
  note pspace_distinct' = invs_pspace_distinct'[OF invs] and
       ...
       pspace_bounded' = invs_pspace_bounded'[OF invs] and
       valid_objs' = invs_valid_objs'[OF invs] and
       valid_untyped'_def2 =
         valid_untyped'[OF pspace_distinct' pspace_aligned' pspace_bounded' al]
  ...
  hence "cpspace_relation ?ks' (underlying_memory (ksMachineState s)) ?th_s"
    ...
    apply -
    apply (elim conjE)
    apply_trace ((subst lift_t_typ_region_bytes,
...
apply (rule cm_disj)
apply assumption+
apply (simp add: objBits_simps' bit_simps scBits_simps
                          heap_to_user_data_restrict heap_to_device_data_restrict)
apply (simp add: objBits_simps' bit_simps scBits_simps
                          heap_to_user_data_restrict heap_to_device_data_restrict)
prefer 2
thm sizeof_sched_context_t_def
find_theorems sched_context_C
apply (simp add: objBits_simps' bit_simps scBits_simps
                          heap_to_user_data_restrict heap_to_device_data_restrict)
apply_trace (simp add: objBits_simps' bit_simps scBits_simps
                          heap_to_user_data_restrict heap_to_device_data_restrict)
find_theorems (149) name: size name: of name: sched
    apply_trace ((subst lift_t_typ_region_bytes,
            rule cm_disj cm_disj_tcb cm_disj_cte cm_disj_user cm_disj_device
            , assumption +,
            simp_all add: objBits_simps' bit_simps scBits_simps
...
apply (clarsimp split: arch_kernel_object.splits)
apply safe[1]
apply (clarsimp simp: objBits_simps' pageBits_def)
apply (clarsimp simp: objBits_simps' pageBits_def pte_bits_def word_size_bits_def)
defer
