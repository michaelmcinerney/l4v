...
lemma messageInfoFromWord_spec:
  "\<forall>s. \<Gamma> \<turnstile> {s} Call messageInfoFromWord_'proc {t. seL4_MessageInfo_lift (ret__struct_seL4_MessageInfo_C_' t) =
            \<lparr>label_CL = (w_' s >> 17) && 0x7FFFFFFFFFFF, capsUnwrapped_CL = (w_' s >> 10) && 0x7F,
...
lemma setExtraBadge_ccorres:
  ...
  apply (subgoal_tac " is_aligned (buffer + (of_nat n * 8 + 0x3B0)) 3")
   apply clarsimp
   prefer 2
   apply (clarsimp simp: valid_ipc_buffer_ptr'_def)
...
next
  ...
  have n7: "n \<le> 7" using Cons.prems by simp
  hence of_nat_n7[intro!]: "of_nat n \<le> (7 :: machine_word)"
  ...
  have scast_2n_eq:
    "n \<le> 6 \<Longrightarrow> SCAST(32 signed \<rightarrow> 64) (1 << n) = (1 << n)"
    apply (case_tac "n=0"; simp)
    apply (case_tac "n=1"; simp)
    apply (case_tac "n=2", fastforce)
    apply (case_tac "n=3", fastforce)
    apply (case_tac "n=4", fastforce)
    apply (case_tac "n=5", fastforce)
    by (case_tac "n=6"; fastforce)
  ...
  show ?thesis
    apply (cinit lift: sender_' receiver_' sendBuffer_' receiveBuffer_'
     ...
     apply (ctac(c_lines 2, no_vcg) add: getMessageInfo_ccorres')
      ...
      apply (wpsimp wp: getMessageInfo_le7 getMessageInfo_msgLength[unfolded K_def] static_imp_wp)+
