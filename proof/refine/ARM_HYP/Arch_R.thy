...
lemma vcpuSwitch_corres'':
  assumes "vcpu' = vcpu"
  shows
  "corres dc (\<lambda>s. (vcpu \<noteq> None \<longrightarrow> vcpu_at  (the vcpu) s) \<and> valid_arch_state s)
...
lemma assoc_invs':
  "\<lbrace>invs' and
   ...
   associateVCPUTCB_helper vcpu v t
   \<lbrace>\<lambda>_. invs'\<rbrace>"
  unfolding associateVCPUTCB_helper_def invs'_def valid_state'_def valid_pspace'_def valid_mdb'_def
...
lemma associateVCPUTCB_helper_rewrite:
  "do archThreadSet (atcbVCPUPtr_update (\<lambda>_. Just v)) tcb;
      setObject v $ vcpuTCBPtr_update (\<lambda>_. Just tcb) vcpu;
      f
   od = do associateVCPUTCB_helper vcpu v tcb;
           f
        od"
  by (simp add: associateVCPUTCB_helper_def bind_assoc)
...
lemma vcpuTCBPtr_update_vcpu_live[wp]:
  "\<lbrace>ko_wp_at' is_vcpu' vcpuPtr\<rbrace>
   setObject vcpuPtr (vcpuTCBPtr_update (\<lambda>_. Some tcb) vcpu)
