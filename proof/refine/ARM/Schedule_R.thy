...
lemma schedule_invs':
  ...
  supply if_split [split del]
  apply (simp add: schedule_def scAndTimer_def)
...
lemma setNextInterrupt_corres:
  "corres dc ((\<lambda>s. active_sc_tcb_at (cur_thread s) s)
              ...
              and valid_objs and active_sc_valid_refills and pspace_aligned and pspace_distinct)
             valid_objs'
             set_next_interrupt
             setNextInterrupt"
...
apply (clarsimp simp: vs_all_heap_simps obj_at_def is_tcb_def is_sc_obj_def)
apply (erule valid_sched_context_size_objsI)
  apply fastforce
apply (clarsimp simp: vs_all_heap_simps obj_at_def is_tcb_def is_sc_obj_def)
apply (clarsimp simp: vs_all_heap_simps obj_at_def is_tcb_def is_sc_obj_def)
apply (clarsimp simp: vs_all_heap_simps obj_at_def is_tcb_def is_sc_obj_def)
...
apply force
apply (clarsimp simp: vs_all_heap_simps obj_at_def is_tcb_def is_sc_obj_def)
  using list.set_sel(1) valid_sched_context_size_objsI apply fastforce
apply (clarsimp simp: vs_all_heap_simps obj_at_def is_tcb_def is_sc_obj_def)
  using list.set_sel(1) valid_sched_context_size_objsI apply fastforce
apply (clarsimp simp: vs_all_heap_simps obj_at_def is_tcb_def is_sc_obj_def)
...
lemma corres_fail':
   ...
   shows "corres_underlying sr nf nf' R P P' f f'"
   using no_fail
...
lemma switchSchedContext_corres:
  "corres dc (\<lambda>s. invs s  \<and> active_sc_valid_refills s \<and> current_time_bounded 1 s
                  \<and> (\<not> cur_sc_tcb_are_bound s \<longrightarrow> active_sc_tcb_at (cur_thread s) s))
             (valid_objs' and cur_tcb')
  ...
  apply (rule corres_symb_exec_l[rotated, OF _ get_sched_context_sp])
    apply (fastforce dest: invs_cur_sc_tcb
                    intro: get_sched_context_exs_valid
                     simp: obj_at_def cur_sc_tcb_def sc_at_pred_n_def)
   apply wpsimp
   apply (fastforce dest: invs_cur_sc_tcb
                    simp: obj_at_def cur_sc_tcb_def sc_at_pred_n_def)
...
thm cur_sc_tcb_def
term valid_sched_control_inv
find_theorems active_sc_tcb_at valid
find_theorems sym_refs  tcb_sched_context -valid
find_theorems ct_running
find_theorems choose_new_thread valid
thm scAndTimer_def setNextInterrupt_corres switchSchedContext_corres


lemma commit_time_active_sc_at_cur_thread[wp]:
  "commit_time \<lbrace>\<lambda>s. active_sc_tcb_at (cur_thread s) s\<rbrace>"
  apply (clarsimp simp: commit_time_def)
  apply wpsimp
  done

crunches if_cond_refill_unblock_check
  for active_sc_at_cur_thread[wp]: "\<lambda>s. active_sc_tcb_at (cur_thread s) s"
  (simp: crunch_simps)

lemma switch_sched_context_active_sc_at_cur_thread[wp]:
  "switch_sched_context  \<lbrace>\<lambda>s. active_sc_tcb_at (cur_thread s) s\<rbrace>"
  apply (clarsimp simp: switch_sched_context_def)
  apply (wpsimp wp: hoare_drop_imps)
  done

find_theorems refill_unblock_check valid_release_q
find_theorems name: active release_queue
thm switch_sched_context_def schedule_valid_sched
...
find_theorems tcb_inv_wf'
thm tcb_inv_wf'_def
lemma scAndTime_corres:
  "corres dc (\<lambda>s. active_sc_tcb_at (cur_thread s) s \<and> invs s \<and> active_sc_valid_refills s
                  \<and> valid_release_q s \<and> current_time_bounded 1 s)
             (valid_objs' and cur_tcb')
             sc_and_timer
             scAndTimer"
apply add_sym_refs
...
apply wpsimp
apply wpsimp
find_theorems switch_sched_context valid_sched
find_theorems active_sc_tcb_at name: switch
thm call_kernel_valid_sched
thm corres_cross_add_guard
thm switch_sched_context_valid_sched schedule_valid_sched
\<comment> \<open>(\<lambda>s. active_sc_tcb_at (cur_thread s) s /\ invs s /\ active_sc_valid_refills  /\ valid_release_q s /\ current_time_bounded 1 s\<close>
...
find_theorems (55) handle_event
find_theorems Schedule_A.schedule
find_theorems (48) schedule_choose_new_thread
