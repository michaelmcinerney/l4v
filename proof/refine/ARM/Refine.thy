...
\<comment> \<open>lemma thread_set_tcb_arch_ct_schedulable[wp]:
...
lemma helperme[simp]:
   "tcb_at t s \<Longrightarrow> ep_send_qs_of_kh (kheap s(t \<mapsto> TCB (f tcb)))
 = ep_send_qs_of_kh (kheap s)"
apply (rule ext)
apply (auto simp: ep_recv_qs_of_eps_def vs_all_heap_simps opt_map_def ep_send_qs_of_eps_def get_tcb_def
eps_of_def eps_of_kh_def ep_of_def send_q_of_def
...
lemma helperme2[simp]:
   "tcb_at t s \<Longrightarrow> ep_recv_qs_of_kh (kheap s(t \<mapsto> TCB (f tcb)))
 = ep_recv_qs_of_kh (kheap s)"
apply (rule ext)
apply (auto simp: ep_recv_qs_of_eps_def vs_all_heap_simps opt_map_def ep_send_qs_of_eps_def get_tcb_def
eps_of_def eps_of_kh_def ep_of_def send_q_of_def
split: option.splits kernel_object.splits if_splits)
...
lemma helperme3[simp]:
   "tcb_at t s \<Longrightarrow> sc_tcbs_of_kh (kheap s(t \<mapsto> TCB (f tcb)))
 = sc_tcbs_of_kh (kheap s)"
apply (auto simp:  vs_all_heap_simps)
done


lemma helperme4[simp]:
   "get_tcb t s = Some tcb \<Longrightarrow> etcbs_of_kh (kheap s(t \<mapsto> TCB ((tcb\<lparr>tcb_arch := f (tcb_arch tcb)\<rparr>))))
 = etcbs_of_kh (kheap s)"

apply (auto simp: etcbs_of_tcbs_def etcb_of_def
map_project_def
 vs_all_heap_simps tcbs_of_kh_def opt_map_def map_project_simps tcb_of_def
get_tcb_def
 split: option.splits Structures_A.kernel_object.splits)
done

lemma helperme5[simp]:
   "get_tcb t s = Some tcb \<Longrightarrow> tcb_sts_of_kh (kheap s(t \<mapsto> TCB ((tcb\<lparr>tcb_arch := f (tcb_arch tcb)\<rparr>))))
 = tcb_sts_of_kh (kheap s)"

apply (auto simp: tcb_sts_of_tcbs_def etcb_of_def
map_project_def
 vs_all_heap_simps tcbs_of_kh_def opt_map_def map_project_simps tcb_of_def
get_tcb_def
 split: option.splits Structures_A.kernel_object.splits)
...
lemma helperme6[simp]:
   "get_tcb t s = Some tcb \<Longrightarrow> tcb_scps_of_kh (kheap s(t \<mapsto> TCB ((tcb\<lparr>tcb_arch := f (tcb_arch tcb)\<rparr>))))
 = tcb_scps_of_kh (kheap s)"

apply (auto simp: tcb_sts_of_tcbs_def etcb_of_def
map_project_def tcb_scps_of_tcbs_def
 vs_all_heap_simps tcbs_of_kh_def opt_map_def map_project_simps tcb_of_def
get_tcb_def
 split: option.splits Structures_A.kernel_object.splits)
done

lemma helperme7[simp]:
   "get_tcb t s = Some tcb \<Longrightarrow> tcb_faults_of_kh (kheap s(t \<mapsto> TCB ((tcb\<lparr>tcb_arch := f (tcb_arch tcb)\<rparr>))))
 = tcb_faults_of_kh (kheap s)"

apply (auto simp: tcb_sts_of_tcbs_def etcb_of_def
map_project_def tcb_scps_of_tcbs_def
 vs_all_heap_simps tcbs_of_kh_def opt_map_def map_project_simps tcb_of_def
get_tcb_def tcb_faults_of_tcbs_def
 split: option.splits Structures_A.kernel_object.splits)
done

lemma helperme8[simp]:
   "tcb_at t s \<Longrightarrow> sc_refill_cfgs_of_kh (kheap s(t \<mapsto> TCB (f tcb)))
 = sc_refill_cfgs_of_kh (kheap s)"
...
lemma helperme9[simp]:
   "tcb_at t s \<Longrightarrow> sc_replies_of_kh (kheap s(t \<mapsto> TCB (f tcb)))
 = sc_replies_of_kh (kheap s)"
...
\<comment> \<open>lemma tcb_arch_update_valid_sched[wp]:
  "thread_set (\<lambda>tcb. tcb\<lparr>tcb_arch := f (tcb_arch tcb)\<rparr>) t \<lbrace>valid_sched_pred_strong P\<rbrace>"
  apply (wpsimp simp:  wp: thread_set_wp hoare_drop_imp)
thm back_subst
apply (subst helperme)
apply (clarsimp simp: get_tcb_def obj_at_def is_tcb_def)
apply (case_tac "kheap s t"; clarsimp)
apply (case_tac a; clarsimp)
apply (subst helperme2)
apply (clarsimp simp: get_tcb_def obj_at_def is_tcb_def)
apply (case_tac "kheap s t"; clarsimp)
apply (case_tac a; clarsimp)
apply (subst helperme3)
apply (clarsimp simp: get_tcb_def obj_at_def is_tcb_def)
apply (case_tac "kheap s t"; clarsimp)
apply (case_tac a; clarsimp)
apply (subst helperme8)
apply (clarsimp simp: get_tcb_def obj_at_def is_tcb_def)
apply (case_tac "kheap s t"; clarsimp)
apply (case_tac a; clarsimp)
apply (subst helperme9)
apply (clarsimp simp: get_tcb_def obj_at_def is_tcb_def)
apply (case_tac "kheap s t"; clarsimp)
apply (case_tac a; clarsimp)
apply simp
...
lemma kernel_entry_invs:
  "\<lbrace>\<lambda>s. einvs s \<and> (e \<noteq> Interrupt \<longrightarrow> ct_running s)
        \<and> (0 < domain_time s) \<and> valid_domain_list s \<and> (ct_running s \<or> ct_idle s)
        \<and> scheduler_action s = resume_cur_thread
        \<and> cur_sc_active s \<and> ct_not_in_release_q s
        \<and> valid_machine_time s \<and> current_time_bounded 5 s \<and> consumed_time_bounded s
        \<and> (cur_sc_offset_ready (consumed_time s) s \<and> cur_sc_offset_sufficient (consumed_time s) s)\<rbrace>
   kernel_entry e us
   \<lbrace>\<lambda>_ s. einvs s
          \<and> (0 < domain_time s) \<and> valid_domain_list s \<and> (ct_running s \<or> ct_idle s)
          \<and> scheduler_action s = resume_cur_thread
          \<and> cur_sc_active s \<and> ct_not_in_release_q s
          \<and> valid_machine_time s \<and> current_time_bounded 5 s \<and> consumed_time_bounded s
          \<and> (cur_sc_offset_ready (consumed_time s) s \<and> cur_sc_offset_sufficient (consumed_time s) s)\<rbrace>"
  apply (rule_tac Q="\<lambda>_ s. (invs s \<and> (ct_running s \<or> ct_idle s)) \<and> valid_sched s
...
apply (rule hoare_vcg_conj_lift_pre_fix)
apply (clarsimp simp: kernel_entry_def)
  apply ((wpsimp wp: call_kernel_domain_list_inv_det_ext

      | clarsimp simp: active_from_running)+)[1]
...
thm call_kernel_schact_is_rct
  ...
  apply (wp thread_set_invs_trivial
             thread_set_not_state_valid_sched
            hoare_vcg_disj_lift ct_in_state_thread_state_lift thread_set_no_change_tcb_state
            static_imp_wp call_kernel_cur_sc_active
      | clarsimp simp add: tcb_cap_cases_def)+
...
\<and> cur_sc_active s \<and>
             ct_not_in_release_q s \<and>
             valid_machine_time s \<and>
             current_time_bounded 5 s \<and>
             consumed_time_bounded s \<and>
             cur_sc_offset_ready (consumed_time s) s \<and> cur_sc_offset_sufficient (consumed_time s) s}"
...
lemma do_user_op_invs2:
  "\<lbrace>\<lambda>s. einvs s \<and> ct_running s \<and> scheduler_action s = resume_cur_thread
        \<and> 0 < domain_time s \<and> valid_domain_list s
\<and> cur_sc_active s \<and>
             ct_not_in_release_q s \<and>
             valid_machine_time s \<and>
   ...
   do_user_op f tc
   \<lbrace>\<lambda>_ s. einvs s \<and> ct_running s \<and> scheduler_action s = resume_cur_thread
          \<and> (0 < domain_time s) \<and> valid_domain_list s
\<and> cur_sc_active s \<and>
             ct_not_in_release_q s \<and>
             valid_machine_time s \<and>
             ...
             consumed_time_bounded s \<and>
             cur_sc_offset_ready (consumed_time s) s \<and> cur_sc_offset_sufficient (consumed_time s) s\<rbrace>"
  apply (rule_tac Q="\<lambda>_ s. valid_list s \<and> valid_sched s \<and>
   scheduler_action s = resume_cur_thread \<and> (invs s \<and> ct_running s) \<and>
   (0 < domain_time s) \<and> valid_domain_list s
\<and> cur_sc_active s \<and>
             ct_not_in_release_q s \<and>
             valid_machine_time s \<and>
             current_time_bounded 5 s \<and>
...
apply (intro hoare_vcg_conj_lift_pre_fix)
  ...
  apply (wp do_user_op_valid_list do_user_op_valid_sched do_user_op_sched_act do_user_op_domain_time
    do_user_op_invs | simp | force)
apply (rule_tac Q="\<lambda>_. invs and ct_running" in hoare_post_imp, fastforce)
apply (wpsimp wp: do_user_op_invs)
apply (rule_tac Q="\<lambda>_. invs and ct_running" in hoare_post_imp, fastforce)
apply (wpsimp wp: do_user_op_invs)
  apply (wp do_user_op_valid_list do_user_op_valid_sched do_user_op_sched_act do_user_op_domain_time
    do_user_op_invs | simp | force)+
...
lemma cur_sc_active_init[simp]:
  ...
  apply (clarsimp simp: init_A_st_def init_kheap_def ext_init_def vs_all_heap_simps active_sc_def MIN_REFILLS_def)
  done
...
apply (clarsimp simp: vs_all_heap_simps init_kheap_def active_sc_def MIN_REFILLS_def
not_in_release_q_2_def valid_machine_time_def init_machine_state_def current_time_bounded_def init_A_st_def)
done
...
lemma cur_sc_offset_ready_and_sufficient[simp]:
  "cur_sc_offset_ready (consumed_time init_A_st) init_A_st \<and>
...
lemma check_active_irq_invs:
  "check_active_irq
   \<lbrace>\<lambda>s. einvs s \<and> (ct_running s \<or> ct_idle s) \<and> scheduler_action s = resume_cur_thread
        \<and> 0 < domain_time s \<and> valid_domain_list s
        \<and> cur_sc_active s \<and>
             ct_not_in_release_q s \<and>
             valid_machine_time s \<and>
...
lemma check_active_irq_invs_just_idle:
  "check_active_irq
   \<lbrace>\<lambda>s. einvs s \<and> (ct_idle s) \<and> scheduler_action s = resume_cur_thread
        \<and> 0 < domain_time s \<and> valid_domain_list s
        \<and> cur_sc_active s \<and>
             ct_not_in_release_q s \<and>
             valid_machine_time s \<and>
...
lemma ckernel_invs:
  ...
  callKernel e
  \<lbrace>\<lambda>rs.
\<comment> \<open>(\<lambda>s. ksSchedulerAction s = ResumeCurrentThread) and\<close>
invs'
\<comment> \<open>and (ct_running' or ct_idle')\<close>
\<rbrace>"
...
\<comment> \<open>and
         (\<lambda>s. 0 < ksDomainTime s) and valid_domain_list' \<close>
\<rbrace>"
...
lemma entry_corres:
  "corres (=) (\<lambda>s. einvs s \<and> (event \<noteq> Interrupt \<longrightarrow> ct_running s)
        \<and> (0 < domain_time s) \<and> valid_domain_list s \<and> (ct_running s \<or> ct_idle s)
        \<and> scheduler_action s = resume_cur_thread
        \<and> cur_sc_active s \<and> ct_not_in_release_q s
        \<and> valid_machine_time s \<and> current_time_bounded 5 s \<and> consumed_time_bounded s
...
definition
  "full_invs' \<equiv> {((tc,s),m,e). invs' s \<and> vs_valid_duplicates' (ksPSpace s) \<and>
                          ex_abs (\<lambda>s :: det_state. einvs s \<and>
scheduler_action s = resume_cur_thread
\<and> valid_domain_list s \<and> (ct_running s \<or> ct_idle s) \<and> (e \<noteq> None \<and> e \<noteq> Some Interrupt \<longrightarrow> ct_running s)
...
\<comment> \<open>\<and>
                          ex_abs (\<lambda>s :: det_state. scheduler_action s = resume_cur_thread
\<and> valid_domain_list s \<and> (ct_running s \<or> ct_idle s) \<and> (e \<noteq> Some Interrupt \<longrightarrow> ct_running s)
...
\<comment> \<open>\<and>                           0 < ksDomainTime s \<and> valid_domain_list' s\<close>
\<comment> \<open>\<and>\<close>
                          \<comment> \<open>ksSchedulerAction s = ResumeCurrentThread \<and>\<close>
\<comment> \<open>                          (ct_running' s \<or> ct_idle' s) \<and>
                          (m = KernelMode \<longrightarrow> e \<noteq> None) \<and>
                          (m = UserMode \<longrightarrow> ct_running' s) \<and>
                          (m = IdleMode \<longrightarrow> ct_idle' s) \<and>
                          (e \<noteq> None \<and> e \<noteq> Some Interrupt \<longrightarrow> ct_running' s) \<and>
                          0 < ksDomainTime s \<and> valid_domain_list' s\<close>
}"
...
lemma checkActiveIRQ_just_running_corres:
  "corres (=)
    (invs and ct_running and einvs and (\<lambda>s. scheduler_action s = resume_cur_thread)
      and (\<lambda>s. 0 < domain_time s) and valid_domain_list)
    (invs'
\<comment> \<open>and ct_running'\<close>
and (\<lambda>s. vs_valid_duplicates' (ksPSpace s))
\<comment> \<open>      and (\<lambda>s. 0 < ksDomainTime s) and valid_domain_list'
      and (\<lambda>s. ksSchedulerAction s = ResumeCurrentThread)\<close>
)
    (check_active_irq) (checkActiveIRQ)"
  apply (rule corres_guard_imp)
    apply (rule checkActiveIRQ_corres', auto)
...
lemma ckernel_invariant:
  ...
  apply (rule invariantI)
   ...
   apply (rule conjI)
    ...
    apply (clarsimp simp: ex_abs_def lift_state_relation_def)
    apply (frule akernel_init_invs[THEN bspec])
    \<comment> \<open>apply (rule_tac x = s in exI)\<close>
...
apply (clarsimp simp: ex_abs_def)
   \<comment> \<open>apply (fastforce simp: Init_H_def valid_domain_list'_def)\<close>
...
apply_trace (clarsimp simp: ex_abs_underlying_def kernel_call_H_def)
apply (prop_tac " ksSchedulerAction bb = ResumeCurrentThread")
apply (frule_tac s=s and s'="bb" in state_relation_schact)
apply (clarsimp simp: sched_act_relation_def)
\<comment> \<open>apply (prop_tac " 0 < ksDomainTime bb")
apply (prop_tac "valid_domain_list' bb")
  apply fastforce
apply (frule_tac s="sa" and s'="bb" in state_relation_schact)
apply (frule_tac a=sa and c=bb and P="\<lambda>t. 0 < t" in domain_time_rel_eq)
  apply fastforce\<close>

find_theorems state_relation domain_time
...
apply (fastforce intro: ct_running_cross ct_idle_cross)
  ...
  apply fastforce
apply (drule use_valid[OF _ valid_corres_combined[OF kernel_entry_invs entry_corres]])
\<comment> \<open>apply (clarsimp simp: ex_abs_underlying_def)

apply fastforce
apply (drule use_valid[OF _ kernelEntry_invs'])
  apply fastforce
  apply fastforce\<close>
\<comment> \<open>apply (drule use_valid[OF _ valid_corres_combined[OF kernel_entry_invs entry_corres]])\<close>
apply (rule hoare_weaken_pre)
apply (rule kernelEntry_invs')

  \<comment> \<open>apply force\<close>
apply clarsimp
apply (intro conjI)
apply (fastforce intro: ct_running_cross ct_idle_cross resume_cur_thread_cross)
apply (elim disjE)
apply (fastforce intro: ct_running_cross ct_idle_cross resume_cur_thread_cross)
apply (fastforce intro: ct_running_cross ct_idle_cross resume_cur_thread_cross)
apply (fastforce intro: ct_running_cross ct_idle_cross resume_cur_thread_cross)
apply (fastforce intro: ct_running_cross ct_idle_cross resume_cur_thread_cross)
apply (rule_tac a=sb and c=sa in resume_cur_thread_cross)
apply fastforce
apply (fastforce intro!:   resume_cur_thread_cross)

\<comment> \<open>apply (fastforce intro: ct_running_cross ct_idle_cross resume_cur_thread_cross)\<close>

apply (rule_tac x=s in exI)
apply clarsimp
apply clarsimp
\<comment> \<open>apply (fastforce intro: ct_running_cross ct_idle_cross)\<close>
\<comment> \<open>apply clarsimp\<close>
apply (rule_tac x=sa in exI)

apply clarsimp
   \<comment> \<open>apply (fastforce intro!:  ct_running_related )\<close>
...
apply clarsimp
apply (intro conjI)
\<comment> \<open>apply (fastforce intro: ct_running_cross ct_idle_cross)

apply clarsimp
apply (rule conjI)\<close>
apply (rule_tac x=sa in exI)

apply clarsimp
apply (prop_tac "ct_running' b \<or> ct_idle' b")

apply (fastforce intro: ct_running_cross ct_idle_cross)
  apply force

\<comment> \<open>   apply (fastforce intro!:  ct_running_related )


thm kernelEntry_invs'
apply (drule use_valid[OF _ valid_corres_combined
                            [OF kernel_entry_invs entry_corres]])
thm valid_corres_combined kernel_entry_invs entry_corres kernelEntry_invs'
thm use_valid[OF _ valid_corres_combined
                            [OF kernel_entry_invs entry_corres],
...
apply clarsimp
find_theorems ksSchedulerAction state_relation
apply (fastforce intro: resume_cur_thread_cross ct_running_cross)

     \<comment> \<open>apply (fastforce simp: ex_abs_def)\<close>
    apply (clarsimp simp: ex_abs_def, rule_tac x=s in exI,
            clarsimp simp: ct_running_related sched_act_rct_related)
   apply (clarsimp simp: ex_abs_def)
apply (rule_tac x=sa in exI)
apply clarsimp
   \<comment> \<open>apply (fastforce simp: ex_abs_def ct_running_related sched_act_rct_related)\<close>
...
\<comment> \<open>
   ...
   apply (drule use_valid)
     apply (rule hoare_vcg_conj_lift)
      apply (rule checkActiveIRQ_valid_duplicates')
apply (rule checkActiveIRQ_invs'_just_running)
apply clarsimp
apply (clarsimp simp: ex_abs_def)
apply (intro conjI)
apply (rule_tac x=s in exI)
...
\<close>
\<comment> \<open>apply (rule valid_corres_combined)\<close>
\<comment> \<open>
  apply blast
subgoal sorry
apply clarsimp

     apply (rule valid_corres_combined)
apply (clarsimp simp: ex_abs_def)


apply (rule check_active_irq_invs_just_running)
apply (rule corres_guard_imp)
apply (rule checkActiveIRQ_just_running_corres)
apply clarsimp
thm checkActiveIRQ_just_running_corres
     apply (rule valid_corres_combined[OF check_active_irq_invs_just_running checkActiveIRQ_just_running_corres])
     apply (rule checkActiveIRQ_invs'_just_running[THEN hoare_weaken_pre])
     apply (fastforce simp: ex_abs_def)
    apply (fastforce simp: ex_abs_def ct_running_related sched_act_rct_related)
   apply (fastforce simp: ex_abs_def)\<close>
...
\<comment> \<open>     \<close>apply (fastforce simp: ex_abs_def)\<comment> \<open>
    apply (fastforce simp: ex_abs_def ct_running_related sched_act_rct_related)
   apply (fastforce simp: ex_abs_def)\<close>

\<comment> \<open>   apply (drule use_valid)
     apply (rule hoare_vcg_conj_lift)
      apply (rule checkActiveIRQ_valid_duplicates')
...
lemma fw_sim_A_H:
  ...
  apply (intro conjI)
   ...
   apply (erule_tac P="a \<and> (\<exists>x. b x)" for a b in disjE)
    apply_trace (clarsimp simp add: kernel_call_H_def kernel_call_A_def)
    apply_trace (rule rev_mp, rule_tac tc=tc and event=x in entry_corres)
...
apply (frule_tac a=b and c="ck'" in ct_running_cross)
  apply fastforce
thm use_valid[OF _ kernel_entry_invs]
...
\<comment> \<open>apply (rule impE)
    ...
    apply (clarsimp simp: ct_in_state_def st_tcb_at_def obj_at_def)\<close>
