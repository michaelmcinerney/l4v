...
lemma st_tcb_at'_valid_idle'_helper':
  "valid_idle' s \<Longrightarrow> \<forall>t. t = ksIdleThread s \<longrightarrow> st_tcb_at' idle' t s"
  by (clarsimp simp: invs'_def valid_idle'_def pred_tcb_at'_def obj_at'_def idle_tcb'_def)
