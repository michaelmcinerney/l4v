  ...
  apply (rule_tac Q ="valid_cap (tcb_fault_handler tcb) and einvs and st_tcb_at active thread and
    ...
    apply (rule corres_guard_imp[OF corres_split'])
         apply (rule corres_split_deprecated[OF sendIPC_corres threadset_corres];
          ...
          by (wp thread_set_invs_trivial thread_set_no_change_tcb_state
              ...
              | fastforce simp: tcb_cap_cases_def tcb_cte_cases_def st_tcb_at_tcb_at inQ_def
...
lemma handleFault_corres:
  "fr f f' \<Longrightarrow>
   corres dc (einvs and st_tcb_at active thread and ex_nonz_cap_to thread and (%_. valid_fault f))
  ...
  apply (rule_tac r'="tcb_relation" in corres_split')
      apply (rule corres_guard_imp[OF getObject_TCB_corres]; fastforce simp: st_tcb_at_tcb_at)
     apply (rule_tac F="valid_fault f" in corres_gen_asm)
     apply (rule_tac Q ="\<lambda>_. tcb_at  thread and pspace_aligned and pspace_distinct"
       ...
       apply (corressimp corres: setThreadState_corres)
      apply (wpsimp wp: getObject_tcb_wp simp: st_tcb_at_tcb_at valid_tcb_objs invs_valid_objs
             | fastforce)+
   apply (drule obj_at_ko_at', clarsimp)
   apply (frule ko_at_valid_objs'; fastforce simp: valid_obj'_def)
  done
...
lemma hf_invs' [wp]:
  ...
  apply (simp add: handleFault_def handleNoFaultHandler_def)
   ...
   apply_trace (clarsimp simp: projectKO_def fail_def projectKO_opt_tcb ex_nonz_cap_to'_def
