...
lemma updateReply_list_refs_of_replies'_inv:
  ...
  apply (rule ext)
  apply (clarsimp simp: list_refs_of_reply'_def obj_at'_def
...
lemma replyUnlink_valid_replies'[wp]:
  ...
  apply (clarsimp simp: valid_reply'_def pred_tcb_at'_def obj_at'_def replyUnlink_assertion_def)
   ...
   apply (rule corres_guard_imp)
    ...
    (* This command is the lynch-pin. This does all useful state-relation lifting
       ...
       apply (force dest!: sc_replies_relation_scReplies_of
                     simp: obj_at'_def opt_map_red vs_heap_simps is_sc_obj obj_at_def)
      apply wpsimp+
  done
...
lemma updateReply_replyNext_None_invs':
  ...
  apply (clarsimp simp: obj_at'_def valid_reply'_def live_reply'_def
   ...
   apply (erule (1) obj_relation_cutsE)
         apply ((clarsimp simp: other_obj_relation_def is_reply obj_at_def
                         split: Structures_A.kernel_object.split_asm if_split_asm
                                RISCV64_A.arch_kernel_obj.split_asm)+)[6]
...
lemma sc_replies_relation_sc_with_reply_None:
  ...
  apply (clarsimp simp: obj_at'_def projectKO_opt_sc)
...
lemma updateReply_sr_inv:
  ...
  apply (clarsimp simp: setReply_def getReply_def getObject_def setObject_def
...
lemma updateReply_ko_at'_other:
  ...
  apply (wpsimp wp: updateReply_wp_all)
  by (clarsimp simp: obj_at'_def ps_clear_upd)
