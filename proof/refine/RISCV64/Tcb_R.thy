          ...
          apply (rule corres_split_nor)
             apply (rule corres_split_nor)
              ...
              apply (rule_tac Q="\<lambda>_. einvs and tcb_at dest" in hoare_post_imp)
               apply (clarsimp simp: invs_def valid_state_def valid_pspace_def valid_sched_weak_strg valid_sched_def)
              prefer 2
...
lemma sp_corres2:
  "corres dc (valid_etcbs and weak_valid_sched_action and cur_tcb and pspace_aligned and pspace_distinct)
             (Invariants_H.valid_queues and valid_queues' and tcb_at' t and
             (\<lambda>s. weak_sch_act_wf (ksSchedulerAction s) s) and valid_objs' and (\<lambda>_. x \<le> maxPriority))
...
lemma cte_map_tcb_5:
  "cte_map (t, tcb_cnode_index 5) = t + 5*2^cteSizeBits"
  by (simp add: cte_map_def tcb_cnode_index_def objBits_defs cte_level_bits_def)
...
lemma threadcontrol_corres_helper4:
  ...
  \<lbrace>\<lambda>x. Invariants_H.valid_queues and valid_queues'\<rbrace>"
  apply (wp
       | strengthen  invs_valid_queues' invs_queues invs_weak_sch_act_wf
       | clarsimp simp: )+
  by (case_tac ac;
...
lemma setPriority_corres':
  ...
  apply (case_tac p_auth; clarsimp)
  by (corressimp corres: setPriority_corres)
...
lemma setMCPriority_corres':
  ...
  apply (case_tac mcp_auth; clarsimp)
  by (corressimp corres: setMCPriority_corres)
...
lemma installTCBFrameCap_corres:
  ...
  apply (rule corres_guard_imp[where P=P and P'=P'
     ...
     apply (rule corres_guard_imp)
       apply (rule corres_split_norE)
          ...
          apply (rule corres_split_nor)
           ...
           apply (wp threadcontrol_corres_helper1[unfolded pred_conj_def])
          apply simp
          apply (wp hoare_drop_imp)
    ...
    apply (rule corres_guard_imp)
     ...
     apply (fastforce simp: cte_wp_at_caps_of_state valid_fault_handler_def emptyable_def is_cap_simps
...
lemma transferCaps_corres:
  ...
  apply (rule corres_guard_imp)
   ...
   subgoal by ((intro conjI
                | clarsimp simp: tcb_at_cte_at_0 tcb_at_cte_at_1[simplified] tcb_at_cte_at_5
                          ...
                          dest!: invs_psp_aligned invs_distinct
                          split: option.splits cap.splits arch_cap.splits)+)[1]
...
lemma tc_invs':
  ...
  by (auto simp: isValidFaultHandler_def isCap_simps isValidVTableRoot_def)
...
| "tcbinv_relation (tcb_invocation.ThreadControl a sl flt_ep mcp prio croot vroot buf) x
    = (\<exists>flt_ep' croot' vroot' sl' buf'.
                        newroot_rel croot croot' \<and> newroot_rel vroot vroot' \<and> newroot_rel flt_ep flt_ep' \<and>
                        ({croot, vroot, flt_ep, option_map undefined buf} \<noteq> {None}
...
| "tcb_inv_wf' (tcbinvocation.Resume t)
             = (tcb_at' t and ex_nonz_cap_to' t)"
| "tcb_inv_wf' (tcbinvocation.ThreadControl t sl fh mcp p croot vroot buf)
             = (tcb_at' t and ex_nonz_cap_to' t and
               ...
               (\<lambda>s. {croot, vroot, fh, option_map undefined buf} \<noteq> {None}
...
lemma invokeTCB_corres:
  ...
  apply (case_tac ti, simp_all only: tcbinv_relation.simps valid_tcb_invocation_def)
      ...
      apply (rename_tac word one two t2 mcp t3 t4 t5 t6 t7 t8 t9 t10 t11)
...
lemma decodeSetSpace_corres:
 ...
 "\<lbrakk> cap_relation cap cap'; list_all2 (\<lambda>(c, sl) (c', sl'). cap_relation c c' \<and> sl' = cte_map sl) extras extras';
  ...
  apply (simp add: decode_set_space_def decodeSetSpace_def Let_def split del: if_split)
  apply (cases "2 \<le> length args \<and> 3 \<le> length extras'")
   apply (clarsimp simp: val_le_length_Cons list_all2_Cons2 split del: if_split)
   apply (simp add: liftE_bindE liftM_def getThreadCSpaceRoot getThreadVSpaceRoot split del: if_split)
   apply (rule corres_guard_imp)
     apply (rule corres_split_deprecated [OF _ slotCapLongRunningDelete_corres])
        apply (rule corres_split_deprecated [OF _ slotCapLongRunningDelete_corres])
           apply (rule corres_split_norE)
              ...
              apply (rule corres_splitEE [OF _ deriveCap_corres])
                  apply (rule corres_split_norE)
                     apply (rule corres_splitEE [OF _ deriveCap_corres])
                         apply (rule corres_split_norE)
                            apply (rule corres_split_norE[OF corres_trivial])
                               apply (clarsimp simp: returnOk_def newroot_rel_def is_cap_simps)
                              apply (unfold unlessE_whenE)
                              apply (rule corres_whenE)
                                apply (clarsimp simp: valid_fault_handler_def isValidFaultHandler_def has_handler_rights_def)
                                apply (case_tac ac; clarsimp simp: is_cap_simps split: bool.split)
                               apply (rule corres_trivial, simp)
                              apply simp
                             apply wp+
                           apply (rule corres_whenE)
                             apply (case_tac vroot_cap'; simp add: is_valid_vtable_root_def
                                                                   RISCV64_H.isValidVTableRoot_def)
                             apply (rename_tac arch_cap)
                             apply (clarsimp, case_tac arch_cap; simp)
                       ...
                       apply (clarsimp simp: cap_map_update_data)
                      apply (clarsimp split del: if_split, rule hoareE_TrueI)
                     apply (clarsimp split del: if_split, rule hoareE_TrueI)
                    apply (rule corres_whenE[OF _ corres_trivial]; simp)
         ...
         apply (clarsimp split del: if_split)
         apply (wp hoare_vcg_if_lift2 hoare_drop_imps)+
...
lemma decodeTCBConfigure_corres:
 ...
 "\<lbrakk> cap_relation cap cap'; list_all2 (\<lambda>(c, sl) (c', sl'). cap_relation c c' \<and> sl' = cte_map sl) extras extras';
  ...
  apply (cases "length args < 3")
  apply (clarsimp split: list.split)
  apply (cases "length extras < 4")
  apply (clarsimp split: list.split simp: list_all2_Cons2)
...
crunches getThreadBufferSlot, getThreadFaultHandlerSlot, setPriority, setMCPriority
...
lemma inv_tcb_IRQInactive:
  ...
  unfolding invokeTCB_def installTCBCap_def installTCBFrameCap_def
  apply (rule hoare_pre)
   apply (wpc |
