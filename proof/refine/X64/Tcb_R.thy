...
lemma setPriority_corres: "corres dc (einvs and tcb_at t) (invs' and tcb_at' t and (\<lambda>_. x \<le> maxPriority))
                     (set_priority t x) (setPriority t x)"
  apply (rule corres_guard_imp)
...
lemma installTCBCap_sch_act_simple:
 ...
 by (wpsimp wp: checkCap_inv assertDerived_wp_weak cteDelete_sch_act_simple
      | wpsimp wp: sch_act_simple_lift
      | rule conjI)+
                     ...
                     apply (rule corres_splitEE [OF _ deriveCap_corres])
                         apply (rule corres_split_norE)
                            apply (rule corres_split_norE[OF corres_trivial])
                             ...
                             apply wp+
                           apply (rule corres_whenE)
                             apply (case_tac vroot_cap'; simp add: is_valid_vtable_root_def isValidVTableRoot_def
                                                                   X64_H.isValidVTableRoot_def)
                             apply (rename_tac arch_cap)
