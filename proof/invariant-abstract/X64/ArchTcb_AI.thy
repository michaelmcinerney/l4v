...
lemma checked_insert_tcb_invs[wp]: (* arch specific *)
  ...
  apply (rule conjI, simp only: ex_cte_cap_wp_to_def)
   ...
   apply (erule cte_wp_atE[OF caps_of_state_cteD])
    apply (clarsimp simp: obj_at_def is_obj_defs tcb_cap_cases_def)
   apply clarsimp
  apply (rule conjI)
   apply clarsimp
  ...
  apply (rule conjI)
   ...
   by (auto simp: checked_insert_is_derived is_cnode_or_valid_arch_def valid_fault_handler_def
...
lemma valid_ipc_buffer_cap:
  ...
  (c = NullCap \<or> (is_nondevice_page_cap c \<and> is_aligned b msg_align_bits))"
  by (auto simp: valid_ipc_buffer_cap_def is_nondevice_page_cap_def
