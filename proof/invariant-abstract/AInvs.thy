...
lemma thread_set_tcb_arch_ct_schedulable[wp]:
  ...
  apply (fastforce simp: schedulable_def is_sc_active_def get_tcb_def ko_atD in_release_queue_def
                  split: option.splits)
  done
...
lemma kernel_entry_invs:
  ...
  apply (wp akernel_invs thread_set_invs_trivial thread_set_ct_in_state select_wp
         ...
         | clarsimp simp: tcb_cap_cases_def schact_is_rct_def)+
  done
...
lemma do_user_op_invs:
  ...
  apply (wp device_update_invs)
          apply (wpsimp wp: select_wp dmo_invs)+
...
crunches sched_context_yield_to, sched_context_bind_tcb, cancel_all_ipc, cancel_all_signals,
         ...
         sched_context_bind_tcb, bind_notification, send_signal, install_tcb_frame_cap,
         install_tcb_cap, do_reply_transfer, invoke_irq_handler, awaken, check_domain_time,
         if_cond_refill_unblock_check, activate_thread, handle_fault, handle_recv, handle_yield,
         handle_interrupt, preemption_path, perform_invocation
