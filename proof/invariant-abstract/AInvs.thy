...
lemma fast_finalise_schact_is_rct_imp_ct_activatable[wp]:
...
crunches deleting_irq_handler
  for ct_activatable[wp]: "\<lambda>s :: det_state. schact_is_rct s \<longrightarrow> ct_in_state activatable s"
...
lemma call_kernel_cur_sc_offset_ready_and_sufficient:
  ...
  apply (clarsimp simp: call_kernel_def)
  apply (simp flip: bind_assoc)
  apply (rule_tac B="\<lambda>_. ?Q" in hoare_seq_ext) apply wpsimp
  apply (rule hoare_seq_ext[OF schedule_cur_sc_offset_ready_and_sufficient])
  apply (rule validE_valid)
  apply (rule handleE_wp)
   apply (subst liftE_validE)
   apply (wpsimp wp: preemption_path_cur_sc_more_than_ready preemption_path_ct_ready_if_schedulable
                     preemption_path_valid_sched preemption_path_current_time_bounded
