...
lemma ct_idle_cur_sc_is_idle_sc_ptr:
  "\<lbrakk>ct_idle s; cur_sc_tcb s; sym_refs (state_refs_of s); valid_idle s; only_idle s; schact_is_rct s\<rbrakk>
   \<Longrightarrow> cur_sc s = idle_sc_ptr"
  apply (frule invs_strengthen_cur_sc_tcb_are_bound; fastforce?)
  apply (prop_tac "cur_thread s = idle_thread s")
   apply (clarsimp simp: invs_def valid_state_def only_idle_def pred_tcb_at_def obj_at_def
                         ct_in_state_def)
  apply (fastforce simp: vs_all_heap_simps valid_idle_def pred_tcb_at_def obj_at_def
                         ct_in_state_def)
  done

lemma ct_idle_cur_sc_active:
  "\<lbrakk>cur_sc_tcb s; sym_refs (state_refs_of s); valid_idle s; only_idle s; schact_is_rct s; ct_idle s\<rbrakk>
   \<Longrightarrow> cur_sc_active s"
  apply (frule ct_idle_cur_sc_is_idle_sc_ptr; fastforce?)
  apply (fastforce dest: invs_valid_idle ct_idle_cur_sc_is_idle_sc_ptr
                   simp: valid_idle_def vs_all_heap_simps MIN_REFILLS_def obj_at_def active_sc_def)
  done
...
lemma ct_idle_ct_not_in_release_q:
  "\<lbrakk>ct_idle s; invs s; valid_release_q s\<rbrakk> \<Longrightarrow> ct_not_in_release_q s"
  apply (frule invs_valid_idle)
  apply (prop_tac "cur_thread s = idle_thread s")
   apply (clarsimp simp: invs_def valid_state_def only_idle_def pred_tcb_at_def obj_at_def
                         ct_in_state_def)
  apply (fastforce dest: it_not_in_release_qI)
  done
